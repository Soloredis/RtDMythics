...
namespace RtDMythics
{
    internal partial class RtDMythics : BaseUnityPlugin
    {
        ...
        private static readonly string[] WeaponCategoryList =
        {
            // Meadows
            "Meadows",
            "Meadows",
        ...
        // Requirement item names, one row per item, same order as WeaponPrefabList.
        ...
        public void CreateItemRecipeConfigs()
        {
            ...
            CreateWeaponRecipeConfigs(ref order);
            CreateArmorRecipeConfigs(ref order);
        ...
        private void CreateWeaponRecipeConfigs(ref int order)
        {
            try
            {
                ...
                for (int i = 0; i < WeaponPrefabList.Length; i++)
                {
                    ...
                    string section = "Item Recipes - " + WeaponCategoryList[i];
                    ...
                    for (int j = 0; j < WeaponReqItems[i].Length; j++)
                    {
                        ...
                        WeaponReqItemConfigs[i][j].SettingChanged += ItemRecipeConfigChanged;
                        WeaponReqAmountConfigs[i][j].SettingChanged += ItemRecipeConfigChanged;
                    }

                    WeaponEnabledConfigs[i].SettingChanged += ItemRecipeConfigChanged;
                }
            }
            catch (Exception arg)
            {
        ...
        {
            WeaponRecipeConfigChanged(sender, e);
            ArmorRecipeConfigChanged(sender, e);
        }

        private void WeaponRecipeConfigChanged(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < WeaponPrefabList.Length; i++)
